...
//    }
	...
	void testDetalleCliente() throws Exception {
		// Given
		when(clienteService.findByIdCliente(1)).thenReturn(Datos.crearCliente001().orElseThrow(null));
		...
		verify(clienteService).findByIdCliente(1);
	}
	...
	public void guardarCliente() throws Exception {
		...
		when(clienteService.createOrUpdateCliente(any())).then(invocation -> {
		...
		mvc.perform(post("/clientes").contentType(MediaType.APPLICATION_JSON)
				...
				.andDo(print()).andExpect(jsonPath("$.cliente.idcliente", is(1)))
				.andExpect(jsonPath("$.cliente.genero").value("MASCULINO"))
				.andExpect(jsonPath("$.cliente.nombre").value("Jorge Lopez"));

		verify(clienteService).createOrUpdateCliente(any());
